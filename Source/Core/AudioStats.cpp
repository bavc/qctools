...
void AudioStats::StatsFromFrame (struct AVFrame* Frame, int, int)
{
    ...
    pkt_pts[x_Current] = Frame->best_effort_timestamp;
...
void AudioStats::TimeStampFromFrame (struct AVFrame* Frame, size_t FramePos)
{
    ...
    int64_t ts=(Frame->best_effort_timestamp == AV_NOPTS_VALUE)?Frame->pkt_dts : Frame->best_effort_timestamp; // Using DTS is PTS is not available
